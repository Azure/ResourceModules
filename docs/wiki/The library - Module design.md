...
- One or multiple template parameters files (`*parameters.json`) that will be used for testing, located in the `.deploymentTests` sub-folder.
...
- Parameter filenames should ideally relate to the content they deploy. For example, a parameter file `min.deploymentTests.json` should be chosen for a parameter file that contains only the minimum set of parameter to deploy the module.
