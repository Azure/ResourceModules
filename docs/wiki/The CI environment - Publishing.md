...
If you decide to use a public registry (Azure Container Registry) to host the CARML modules, this can be accomplished by setting the `bicepRegistryPublic` environment variable found in the settings.yml file to true. This will enable the anonymous access feature on the ACR, turning it from a private registry to a public registry. Anonymous access on the ACR will allow others to call CARML modules from the ACR using the ACR's login server address along with the path to the module. Example : `br:carml.azurecr.io/bicep/modules/microsoft.storage.storageaccounts:latest`
