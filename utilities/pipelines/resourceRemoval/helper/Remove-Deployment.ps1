...
function Remove-Deployment {
    ...
    process {
        ...
        Write-Verbose ('Remove-Deployment: Total number of deployments after fetching deployments [{0}]' -f $deploymentResourceIds.Count) -Verbose
        ...
        Write-Verbose ('Remove-Deployment: Total number of deployments after pre-filtering & ordering items [{0}]' -f $rawResourceIdsToRemove.Count) -Verbose

        # Format items
        # ============
        $resourcesToRemove = Get-ResourceIdsAsFormattedObjectList -ResourceIds $rawResourceIdsToRemove
        Write-Verbose ('Remove-Deployment: Total number of deployments after formatting items [{0}]' -f $resourcesToRemove.Count) -Verbose
        ...
        Write-Verbose ('Remove-Deployment: Total number of deployments after filtering all dependency resources [{0}]' -f $resourcesToRemove.Count) -Verbose

        # Order resources
        # ===============
        $resourcesToRemove = Get-OrderedResourcesList -ResourcesToOrder $resourcesToRemove -Order $RemovalSequence
        Write-Verbose ('Remove-Deployment: Total number of deployments after final ordering of resources [{0}]' -f $resourcesToRemove.Count) -Verbose
