...
function Find-TemplateFile {
    ...
    $TemplateFilePath = Join-Path $FolderPath 'readme.md'
...
function Get-TemplateFileToPublish {
    ...
    if ($TemplateFilesToPublish.Count -eq 0) {
        Write-Verbose 'No readme file found in the modified module.' -Verbose
    }
    ...
    $TemplateFilesToPublish | ForEach-Object {
        ...
        $RelPath = $RelPath.Split('/readme.')[0]
...
function Get-ParentModuleTemplateFile {
    ...
    #Prioritizing the bicep file
    $ParentTemplateFilePath = Join-Path $ParentFolderPath 'readme.md'
    # if (-not (Test-Path $TemplateFilePath)) {
...
function Get-ModulesToPublish {
    ...
    if ($ModulesToPublish.count -gt 0) {
        Write-Verbose 'Publish the following readmes:'-Verbose
        ...
        Write-Verbose 'No readme to publish.'-Verbose
    }
