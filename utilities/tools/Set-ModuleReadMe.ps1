...
function Set-ModuleReadMe {
    ...
    if ($fullModuleIdentifier.Contains('--')) {
        $fullModuleIdentifierHyphen = $fullModuleIdentifier.Replace('--', '_')
        Write-Verbose ("fullModuleIdentifier: $fullModuleIdentifier") -Verbose
        $splitHyphens = $fullModuleIdentifierHyphen.split('-')
        ...
        $fullResourceType = 'Microsoft.{0}' -f $splitHyphens.Replace('-', '').Replace('_', '-')
