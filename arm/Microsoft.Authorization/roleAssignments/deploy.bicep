...
@description('Optional. Name of the Resource Group to assign the RBAC role(s) to. If no Resource Group name is provided, the module deploys at subscription level, therefore assigns the provided RBAC role(s) to the subscription.')
param resourceGroupName string = ''

@description('Optional. ID of the Subscription to assign the RBAC role(s) to. If no Resource Group name is provided, the module deploys at subscription level, therefore assigns the provided RBAC role(s) to the subscription.')
param subscriptionId string = ''

@description('Optional. ID of the Management Group to assign the RBAC role(s) to. If no Subscription is provided, the module deploys at management group level, therefore assigns the provided RBAC role(s) to the management group.')
...
module roleAssignment_mg './.bicep/nested_roleAssignments_mg.bicep' = if (!empty(managementGroupId) && empty(subscriptionId) && empty(resourceGroupName)) {
  name: 'roleAssignment-mg-${guid(roleDefinitionId_var,principalId)}'
...
module roleAssignment_sub './.bicep/nested_roleAssignments_sub.bicep' = if (empty(managementGroupId) && !empty(subscriptionId) && empty(resourceGroupName)) {
  name: 'roleAssignment-sub-${guid(roleDefinitionId_var,principalId)}'
...
module roleAssignment_rg './.bicep/nested_roleAssignments_rg.bicep' = if (empty(managementGroupId) && !empty(resourceGroupName) && !empty(subscriptionId)) {
  name: 'roleAssignment-${guid(roleDefinitionId_var,principalId)}'
