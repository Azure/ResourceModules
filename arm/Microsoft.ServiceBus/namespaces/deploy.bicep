...
module serviceBusNamespace_queues 'queues/deploy.bicep' = [for (queue, index) in queues: {
  ...
  params: {
    ...
    locks: contains(queue, 'locks') ? queue.locks : locks
...
module serviceBusNamespace_topics 'topics/deploy.bicep' = [for (topic, index) in topics: {
  ...
  params: {
    ...
    locks: contains(topic, 'locks') ? topic.locks : locks
...
resource serviceBusNamespace_locks 'Microsoft.Authorization/locks@2017-04-01' = [for lock in locks: {
