...
module automationAccount_privateEndpoints '../../Microsoft.Network/privateEndpoints/deploy.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
  ...
  params: {
    ...
    name: contains(privateEndpoint, 'name') ? privateEndpoint.name : '${last(split(serviceBusNamespace.id, '/'))}-${split(privateEndpoint.subnetResourceId, '/')[8]}_${last(split(privateEndpoint.subnetResourceId, '/'))}'
