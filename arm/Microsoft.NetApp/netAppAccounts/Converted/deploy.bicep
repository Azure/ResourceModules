...
resource netAppAccountName_resource 'Microsoft.NetApp/netAppAccounts@2021-04-01' = {
...
resource netAppAccountName_Microsoft_Authorization_netAppAccountsDoNotDelete 'Microsoft.NetApp/netAppAccounts/providers/locks@2016-09-01' = if (lockForDeletion) {
  name: '${netAppAccountName}/Microsoft.Authorization/netAppAccountsDoNotDelete'
  properties: {
    level: 'CannotDelete'
  }
  dependsOn: [
...
module name_location_ANF_Account_Rbac './nested_name_location_ANF_Account_Rbac.bicep' = [for (item, i) in roleAssignments: {
  name: '${uniqueString(deployment().name, location)}-ANF-Account-Rbac-${i}'
  params: {
    roleAssignment: item
...
resource netAppAccountName_capacityPools_dummy_capacityPools_poolName 'Microsoft.NetApp/netAppAccounts/capacityPools@2021-04-01' = [for item in capacityPools: if (!empty(capacityPools)) {
  name: '${netAppAccountName}/${(empty(capacityPools) ? 'dummy' : item.poolName)}'
  tags: tags
  location: location
  properties: {
    serviceLevel: item.poolServiceLevel
    size: item.poolSize
  }
  dependsOn: [
    netAppAccountName_resource
...
resource netAppAccountName_capacityPools_dummy_capacityPools_poolName_Microsoft_Authorization_capacityPoolsDoNotDelete 'Microsoft.NetApp/netAppAccounts/capacityPools/providers/locks@2016-09-01' = [for item in capacityPools: if (!empty(capacityPools)) {
  name: '${netAppAccountName}/${(empty(capacityPools) ? 'dummy' : item.poolName)}/Microsoft.Authorization/capacityPoolsDoNotDelete'
  properties: {
    level: 'CannotDelete'
  }
  dependsOn: [
    'Microsoft.NetApp/netAppAccounts/${netAppAccountName}/capacityPools/${(empty(capacityPools) ? 'dummy' : item.poolName)}'
  ]
}]

@batchSize(1)
module name_location_ANF_CapPool_capacityPools_dummy './nested_name_location_ANF_CapPool_capacityPools_dummy.bicep' = [for (item, i) in capacityPools: {
  name: '${uniqueString(deployment().name, location)}-ANF-CapPool-${(empty(capacityPools) ? 'dummy' : i)}'
  params: {
    netAppAccountName: netAppAccountName
    location: location
  ...
  dependsOn: [
    netAppAccountName_capacityPools_dummy_capacityPools_poolName
  ]
}]

output netAppAccountName string = netAppAccountName
output netAppAccountResourceGroup string = resourceGroup().name
output netAppAccountResourceId string = netAppAccountName_resource.id