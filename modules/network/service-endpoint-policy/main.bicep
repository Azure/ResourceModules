...
resource serviceEndpointPolicy_lock 'Microsoft.Authorization/locks@2020-05-01' = if (!empty(lock)) {
  name: '${serviceEndpointPolicy.name}-${lock}-lock'
  properties: {
    level: any(lock)
...
module serviceEndpointPolicy_roleAssignments '.bicep/nested_roleAssignments.bicep' = [for (roleAssignment, index) in roleAssignments: {
  ...
  params: {
    ...
    resourceId: serviceEndpointPolicy.id
  }
}]
