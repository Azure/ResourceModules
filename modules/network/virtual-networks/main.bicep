...
@description('Required. The Virtual Network (vNet) Name. Dummy addition to trigger publishing3.')
param name string
...
module virtualNetwork_peering_local 'virtual-network-peerings/main.bicep' = [for (peering, index) in peerings: {
...
module virtualNetwork_peering_remote 'virtual-network-peerings/main.bicep' = [for (peering, index) in peerings: if (contains(peering, 'remotePeeringEnabled') ? peering.remotePeeringEnabled == true : false) {
