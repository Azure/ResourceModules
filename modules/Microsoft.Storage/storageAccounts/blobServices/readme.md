...
| `changeFeedRetentionInDays` | int | `7` |  | Indicates whether change feed event logging is enabled for the Blob service. Indicates the duration of changeFeed retention in days. Minimum value is 1 day and maximum value is 146000 days (400 years). A "0" value indicates an infinite retention of the change feed. |
...
| `containerDeleteRetentionPolicyDays` | int | `7` |  | Indicates the number of days that the deleted item should be retained. The minimum specified value can be 1 and the maximum value can be 365. |
...
| `deleteRetentionPolicyEnabled` | bool | `True` |  | The blob service properties for blob soft delete. Indicates whether DeleteRetentionPolicy is enabled for the Blob service. |
